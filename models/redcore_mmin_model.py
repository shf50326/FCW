...
class redcoreMMINModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # self.model_names = ['A', 'V', 'L', 'C', 'AE', 'AE_cycle', 'Cls', 'xenc_al2v', 'xenc_av2l', 'xenc_vl2a', 'Cls_a',
        #                     'Cls_v', 'Cls_l']
        self.model_names = ['A', 'V', 'L', 'Cls', 'xenc_al2v', 'xenc_av2l', 'xenc_vl2a', 'Cls_a',
                            'Cls_v', 'Cls_l']
        ...
        #                          use_bn=opt.bn)
        self.feature_dim = 32
        self.netxenc_al2v = ResidualXE(AE_layers, opt.n_blocks, opt.embd_size_a + opt.embd_size_l, opt.embd_size_v,
                                       dropout=0, use_bn=False)
        self.netxenc_av2l = ResidualXE(AE_layers, opt.n_blocks, opt.embd_size_a + opt.embd_size_v, opt.embd_size_l,
                                       dropout=0, use_bn=False)
        self.netxenc_vl2a = ResidualXE(AE_layers, opt.n_blocks, opt.embd_size_v + opt.embd_size_l, opt.embd_size_a,
                                       dropout=0, use_bn=False)
        self.netCls = FcClassifier(opt.embd_size_a + opt.embd_size_l + opt.embd_size_v, cls_layers,
                                   output_dim=opt.output_dim, dropout=opt.dropout_rate, use_bn=opt.bn)
        self.netCls_a = FcClassifier(opt.embd_size_a, cls_layers, output_dim=opt.output_dim, dropout=opt.dropout_rate,
                                     use_bn=opt.bn)
        self.netCls_v = FcClassifier(opt.embd_size_v, cls_layers, output_dim=opt.output_dim, dropout=opt.dropout_rate,
                                     use_bn=opt.bn)
        self.netCls_l = FcClassifier(opt.embd_size_l, cls_layers, output_dim=opt.output_dim, dropout=opt.dropout_rate,
                                     use_bn=opt.bn)
    ...
    def forward(self):
        ...
        self.gen_a, _ = self.netxenc_vl2a(torch.cat([self.feat_V_miss, self.feat_L_miss], dim=-1))
        self.gen_l, _ = self.netxenc_av2l(torch.cat([self.feat_A_miss, self.feat_V_miss], dim=-1))
        self.gen_v, _ = self.netxenc_al2v(torch.cat([self.feat_A_miss, self.feat_L_miss], dim=-1))

        bs = self.feat_A_miss.shape[0]
        self.feat_A_r = self.A_miss_index.reshape(bs, 1) * self.feat_A_miss - (
                self.A_miss_index.reshape(bs, 1) - 1) * self.gen_a
        self.feat_L_r = self.L_miss_index.reshape(bs, 1) * self.feat_L_miss - (
                self.L_miss_index.reshape(bs, 1) - 1) * self.gen_l
        self.feat_V_r = self.V_miss_index.reshape(bs, 1) * self.feat_V_miss - (
    ...
    def backward(self):
        ...
        kld_fa = -1 * lambda1 * torch.sum(
            (1 + self.flogvar_a - self.fmu_a.pow(2) - self.flogvar_a.exp()) * a_index) / bs
        kld_fv = -1 * lambda1 * torch.sum(
            (1 + self.flogvar_v - self.fmu_v.pow(2) - self.flogvar_v.exp()) * v_index) / bs
        kld_fl = -1 * lambda1 * torch.sum(
            (1 + self.flogvar_l - self.fmu_l.pow(2) - self.flogvar_l.exp()) * l_index) / bs
        ...
        self.loss_mse = self.mse_weight * (
                self.beta[0] * self.lossA_r + self.beta[1] * self.lossV_r + self.beta[2] * self.lossL_r)

        self.loss_cycle = 0

        loss = self.loss_CE + (kld_fa + kld_fv + kld_fl) + (
                self.loss_CE_a + self.loss_CE_v + self.loss_CE_l) + self.loss_mse
        # print('loss:', loss)
    ...
    def backward_fcw(self, loss_fcw):
        ...
        loss = self.loss_CE + (kld_fa + kld_fv + kld_fl) + (
                self.loss_CE_a + self.loss_CE_v + self.loss_CE_l) + self.loss_mse + loss_fcw * lamda
        # loss = self.loss_CE + (kld_fa + kld_fv + kld_fl) + (
        #         self.loss_CE_a + self.loss_CE_v + self.loss_CE_l) + self.loss_mse
    ...
    def optimize_parameters_fcw(self, loss_fcw):
        """Calculate losses, gradients, and update network weights; called in every training iteration"""
        # forward
        self.forward()
        # backward
        self.optimizer.zero_grad()
        self.backward_fcw(loss_fcw)
        self.optimizer.step()
        # with self.warmup_scheduler.dampening():
        #    self.lr_scheduler.step()
